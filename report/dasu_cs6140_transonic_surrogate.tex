...
\small \url{https://github.com/ashishdasu/transonic-aerodynamic-surrogate-modeling}}
...
TransonicSurrogate dataset and evaluated GP regression \cite{rasmussen2006},
...
airfoils. That $R^2=0.996$ is an interpolation result: RAE2822 appears in
both training and test data, so the model only needs to generalize to
held-out \emph{operating points} on a geometry it already knows---not to
an unseen airfoil family.

This work revisits the same dataset with a more rigorous evaluation and
substantially deeper analysis. We withhold all 172 RAE2822 samples from
training entirely---a \emph{geometry holdout} that tests generalization to
an unseen airfoil family rather than interpolation within known ones. Every
model fails this test ($R^2 < 0$), a result the original evaluation could
not reveal. Beyond the holdout, we train four surrogates: a polynomial baseline
to quantify where smooth global fits break down, random forest and XGBoost
as the tree-based methods the paper used, and a PyTorch MLP \cite{paszke2019}
with a shared trunk and dual $C_l$/$C_m$ heads as a fully continuous
deep-learning alternative. We omit GP regression (the paper's strongest
model) because its $O(n^3)$ scaling conflicts with the millisecond-inference
goal of a deployed surrogate. We also run four analyses the paper never
performed: leave-one-airfoil-out generalization, data efficiency curves,
Mach-regime extrapolation, and feature ablation.
...
The validation split is used only for hyperparameter selection; both test
splits are evaluated exactly once, after all tuning is done.
...
\begin{table}[ht]
  ...
  \input{../results/tables/test_metrics.tex}
\end{table}
...
\begin{table}[ht]
  ...
  \input{../results/tables/heldout_metrics.tex}
\end{table}

\subsection{Parity and Error Analysis}

Figure~\ref{fig:parity} shows predicted vs.\ CFD-true scatter on the
interpolation test split. XGBoost and the MLP cluster tightly around the
diagonal; the polynomial baseline shows fan-shaped scatter for $C_m$
near $M_\infty=0.80$; RF falls between the two.
...
Figure~\ref{fig:residuals_mach} decomposes RMSE by Mach bin: all models
degrade monotonically toward $M_\infty=0.85$. The polynomial spike at
$M=0.85$ ($\text{RMSE}_{C_l} \approx 0.045$) is $2\times$ its lower-Mach
baseline; tree models and the DNN track the same trend at much lower absolute
error. Model ranking is \emph{preserved} across all five Mach values,
so XGBoost's interpolation lead is not a low-Mach artifact.
...
Among all 18 features, $\alpha$ and $M_\infty$ dominate, which is expected---they directly set the shock strength. Of the geometric coordinates, $y_{U_1}$ and $y_{U_2}$
(leading-edge upper-surface curvature) rank highest, consistent with shock
formation originating at the leading edge. RF and XGBoost
agree on this ranking, which suggests both are responding to the same
physical signal rather than dataset-specific noise.
...
The four analyses below each probe a dimension of generalization that the
source paper's random-split evaluation cannot reveal.

\subsection{Leave-One-Airfoil-Out Generalization}

To assess how broadly the extrapolation failure generalizes, we run a
leave-one-airfoil-out (LOAO) sweep: for each of the eight airfoils, hold
it out entirely, retrain the three classical surrogates on the remaining
seven with fixed best hyperparameters, and evaluate on the held-out set.
Fig.~\ref{fig:loao_bar} reveals \emph{three} generalization regimes.
The 5-digit NACA profiles (NACA23012, NACA24112, NACA25112) cross-generalize
well ($R^2 > 0.95$) because the other family members cover the held-out geometry.
NACA0012 (only symmetric profile) and NACA4412 (highest 4-digit camber) collapse
($R^2(C_l) = -4.2$ and $-6.0$); both RAE profiles reach strongly negative $R^2$.
Extrapolation failure is not specific to supercritical profiles---any airfoil
whose geometry has no near-neighbor in training will fail.
...
extrapolation. Unlike the geometry holdout, this does not produce negative $R^2$:
RF and XGBoost achieve $R^2(C_l)\approx0.90$; the DNN reaches
$R^2(C_m)=0.942$, outperforming XGBoost on that target.
Mach extrapolation is a smaller leap---aerodynamic behavior changes smoothly as
speed increases, so the step from $M=0.80$ to $M=0.85$ is tractable.
A new airfoil geometry is a fundamentally different region of input space,
which is why that test collapses entirely. The polynomial baseline degrades most
($R^2(C_l)=0.75$), confirming its inability to handle near-critical shock nonlinearity.
...
\begin{figure}[ht]
  ...
  \begin{minipage}[c]{0.42\textwidth}
    ...
    \input{../results/tables/mach_extrap_metrics.tex}
  \end{minipage}
\end{figure}
...
and three surface coordinates capture virtually all predictive signal for
well-performing models. Table~\ref{tab:ablation} quantifies the per-model
$\Delta R^2$.
...
\paragraph{Why the models fail on new geometry.}
The source paper's $R^2=0.996$ is real, but it measures the wrong thing:
RAE2822 rows appear in both training and test, so the model only needs to
interpolate within geometries it has already seen. Our geometry holdout
tests something harder and more practically relevant --- can the surrogate
predict for a profile it has never encountered? The answer, for every model,
is no. RAE2822 is a supercritical airfoil whose $C_m$ curve is qualitatively
different from the NACA profiles that dominate training; the 16 surface
coordinates encode shape, but the learned mapping is bound to the training
distribution and cannot extrapolate across that gap. The LOAO sweep
confirms this is not a quirk of RAE2822 specifically: NACA0012 (the only
symmetric profile) and NACA4412 (highest camber among 4-digit profiles)
collapse just as badly when held out, because they too have geometric
properties with no near-neighbors in the remaining training data.
The original paper's random-split evaluation never surfaced any of this
because RAE2822 rows were always present in training---which means the model
never actually had to generalize to a new geometry, it just had to memorize
a new operating point on one it already knew.

\paragraph{Limitations and future work.}
The dataset is inviscid (Euler), 2D, and covers only 8 airfoil families.
The core gap exposed here is that a 16-coordinate shape parameterization
does not generalize across families; geometry-aware encoders---Fourier
descriptors or graph networks on the airfoil mesh---would give the model
a more transferable shape representation. Any future benchmark should use
geometry-stratified cross-validation rather than random splits. Transfer
learning---pre-train on NACA profiles, fine-tune on a handful of new CFD
runs---could close the holdout gap without a full retraining sweep.
