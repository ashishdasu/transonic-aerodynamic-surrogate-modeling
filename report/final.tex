...
\textbf{Keywords:} machine learning, regression, transonic aerodynamics,
surrogate modeling, airfoil, CFD, deep neural networks.
...
Elrefaie et al.\ \cite{elrefaie2024} introduced the TransonicSurrogate
dataset and compared GP regression, random forest, and gradient-boosted trees,
...
fail; (2) we add a PyTorch MLP (required for the multi-output joint learning
analysis); and (3) we introduce a \emph{geometry holdout} that separates
...
infer geometry solely from the continuous coordinates. The prediction targets
are $\mathbf{y} = [C_l,\, C_m]$, framed as multi-output regression. All inputs
and targets are z-score normalized; scalers are fit on the training split only.
...
regularization. This model serves as the \emph{smooth-approximator baseline}
whose failure on shock-induced nonlinearity motivates the non-parametric models.

\subsection{Random Forest}

\texttt{sklearn.MultiOutputRegressor} wrapping
\texttt{RandomForestRegressor} with grid $n_\text{est} \in \{50,100,200\}$,
$d_\text{max} \in \{5, 10, \texttt{None}\}$, 5-fold CV (neg-RMSE objective).
Best: 200 trees, unlimited depth. Predicts both $C_l$ and $C_m$ from
a single forest via implicit multi-output support.

\subsection{XGBoost}

XGBoost does not natively support multi-output regression; we fit
one \texttt{XGBRegressor} per target and wrap both in an
...
independent linear heads for $C_l$ and $C_m$. The shared trunk forces the
model to learn a geometry--flow representation useful for both coefficients
simultaneously. BatchNorm stabilises training by normalising intermediate
activations; dropout ($p=0.1$ in the best config) acts as a regularizer
without over-shrinking representations on this moderately-sized dataset.

We search 20 configurations: five hidden-layer widths
($[128,128]$, $[256,128]$, $[256,256]$, $[256,128,64]$, $[512,256,128]$),
two dropout rates ($p \in \{0.1, 0.3\}$), and two learning rates
($\eta \in \{10^{-3}, 5\times10^{-4}\}$). Training uses Adam (MSE loss),
\texttt{ReduceLROnPlateau} (decay 0.5, patience~10), and early stopping
(patience~20, max 300 epochs). The winning configuration is $[512,256,128]$
with $p=0.1$, indicating the dataset benefits from capacity over shrinkage.
Fig.~\ref{fig:dnn_curves} shows convergence across three seeds (0, 1, 42):
validation RMSE $0.0571$, $0.0631$, $0.0594$ (mean $\pm$ std:
$0.060 \pm 0.003$), confirming training stability across initialisations.
...
hardest physical test: the model must predict the nose-down moment of a
supercritical profile it never encountered in training. XGBoost and the MLP
traces follow the CFD curve more closely than the polynomial baseline, which
deviates noticeably above $\alpha \approx 5^\circ$ where the shock-induced
moment shift becomes pronounced.
...
consistent with leading-edge geometry driving the critical Mach number and
shock onset. The agreement between RF and XGBoost importance rankings
validates that both ensemble methods capture the same physical signal.
...
extrapolation, and inference latency into a single scorecard. The same
degradation pattern holds for \emph{every} model---near-unity interpolation,
catastrophic geometry holdout, moderate Mach extrapolation---confirming that
the failure is a property of the training distribution, not model capacity.
All four surrogates complete a forward pass in $\lesssim 4$\,ms vs.\
...
$-24.3$). The LOAO therefore reveals that extrapolation failure is not
specific to supercritical geometries---any airfoil with a unique geometric
property absent from the training distribution will fail.
...
$R^2(C_m)=0.942$ (outperforming XGBoost on this target). The Mach dimension is
continuous and the gradient from $M=0.80$ to $M=0.85$ is smaller in function
space than the geometry gap between NACA and RAE2822. The polynomial baseline
...
omitted geometric coordinates. These results strongly validate the source
paper's feature selection: $\alpha$, $M_\infty$, and three surface coordinates
capture virtually all predictive signal for the models that matter. The full
18-feature parameterization provides no measurable benefit for tree-based or
neural-network surrogates on this dataset.
...
honest estimates of accuracy on novel profiles.
